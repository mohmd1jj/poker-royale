...
<body>
  ...
  <script>
    ...
    socket.on("connect",()=>{connectionStatus.textContent="Connected";connectionStatus.style.color="#22c55e";addLog("Connected.")}); socket.on("connect_error",(error)=>{connectionStatus.textContent="Socket error";connectionStatus.style.color="#ef4444";addLog("Socket error: "+(error && error.message ? error.message : "connection failed"));}); socket.on("disconnect",(reason)=>{connectionStatus.textContent="Disconnected";connectionStatus.style.color="#ef4444";addLog("Disconnected: "+reason);updateButtons(null)}); socket.on("onlineCount",count=>onlineCount.textContent=count); socket.on("roomsUpdate",rooms=>renderRooms(rooms));
