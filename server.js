...
<body>
  ...
  <div class="app">
    ...
    <div class="main-layout"><aside class="panel"><h2>Lobby Rooms</h2><div id="rooms"></div><h2 style="margin-top:16px;">Leaderboard</h2><div id="leaderboardList" class="leaderboard-list"></div><h2 style="margin-top:16px;">Game History</h2><div id="historyList" class="history-list"></div></aside><main><div class="poker-table"><div class="table-line"></div><div class="dealer">DEALER</div><div class="community" id="communityCards"><div class="card back">â </div><div class="card back">â¥</div><div class="card back">â¦</div><div class="card back">â£</div><div class="card back">â</div></div><div class="pot" id="potDisplay">POT: 0</div><div class="turn-status" id="turnStatus">Login first, then choose a room</div><div class="my-cards" id="myCards"></div><div id="players"></div></div><div class="log" id="gameLog"><div class="log-item">Welcome to Poker Royale.</div></div></main></div>
  </div>
  ...
  <script>
    ...
    logoutBtn.onclick=async()=>{await api("/api/logout",{});currentUser=null;joined=false;currentRoomId=null;latestRoom=null;updateUserPanel();updateButtons(null);playersEl.innerHTML="";myCardsEl.innerHTML="";turnStatus.textContent=tr().loginFirst;addLog("Logged out.");showToast("Logged out")};
    ...
    function cardIsRed(card){return card.includes("â¥")||card.includes("â¦")} function renderCard(card){if(!card)return '<div class="card back">â</div>';return '<div class="card'+(cardIsRed(card)?" red":"")+'">'+card+'</div>'}
    ...
    socket.on("roomJoined",room=>{joined=true;latestRoom=room;currentRoomId=room.id;updateTableText(room);renderPlayers(room.players);renderCommunity(room.communityCards);updateButtons(room);addLog(tr().joined+" "+room.name);showToast(tr().joined+" "+room.name)});
    socket.on("roomState",room=>{if(!currentRoomId||room.id!==currentRoomId)return;latestRoom=room;updateTableText(room);renderPlayers(room.players);renderCommunity(room.communityCards);updateButtons(room)}); socket.on("privateCards",cards=>renderMyCards(cards)); socket.on("gameMessage",async message=>{addLog(message);showToast(message);try{await loadMe();await refreshDashboard()}catch(e){}});
