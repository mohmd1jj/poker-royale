...
function productCard(p){
  return `<article class="product-card reveal">
    ...
    <div class="product-info">
      ...
      <div class="product-price"><strong>${money(p.price)}</strong>${p.oldPrice ? `<span>${money(p.oldPrice)}</span>` : ""}</div>
...
:root{--paper:#fffaf4;--ink:#14110f;--muted:#7e766d;--line:rgba(20,17,15,.12);--gold:#b99558;--black:#101010;--danger:#9f2a2a;--green:#16835a;--shadow:0 22px 70px rgba(76,48,22,.12)}
@keyframes fadeIn{from{opacity:0}to{opacity:1}}
@keyframes floatIn{from{opacity:0;transform:translateY(18px)}to{opacity:1;transform:translateY(0)}}
@keyframes pulse{0%,100%{box-shadow:0 0 0 rgba(185,149,88,0)}50%{box-shadow:0 0 28px rgba(185,149,88,.28)}}
...
a{text-decoration:none;color:inherit}.wrap{width:100%;max-width:1220px;margin:0 auto;padding:0 14px}
.header{position:sticky;top:0;z-index:60;background:rgba(255,250,244,.94);backdrop-filter:blur(18px);border-bottom:1px solid var(--line)}
.header-inner{max-width:1220px;margin:0 auto;display:grid;grid-template-columns:1fr auto 1fr;align-items:center;gap:12px;padding:13px 14px}
.left-actions{display:flex;justify-content:flex-start}.right-actions{display:flex;justify-content:flex-end;gap:8px}.center-brand{text-align:center}
.brand-mark{display:inline-grid;place-items:center;width:54px;height:54px;border-radius:19px;background:linear-gradient(135deg,#14110f,#4a3a22 58%,#d8bd7b);color:#fffaf4;font-family:Georgia,serif;font-size:32px;margin-bottom:5px;animation:pulse 3s ease-in-out infinite}
.brand{display:block;font-family:Georgia,serif;font-size:36px;letter-spacing:6px;line-height:1}.brand-tag{font-size:10px;color:var(--muted);letter-spacing:2px;margin-top:5px}
...
.hamburger,.icon{width:46px;height:46px;border:1px solid var(--line);background:#fffaf4;border-radius:16px;display:grid;place-items:center;font-size:22px;font-weight:900}.icon{font-size:18px;position:relative}.cart-count{position:absolute;top:-5px;left:-5px;background:#101010;color:#fff;border-radius:999px;font-size:10px;width:18px;height:18px;display:grid;place-items:center}

/* GUARANTEED HOME SLIDER */
.home-slider{width:100%;max-width:1220px;margin:20px auto 26px;border-radius:34px;overflow:hidden;box-shadow:var(--shadow);position:relative;background:#101010;min-height:520px}
.home-slide{display:none;min-height:520px;background-size:cover;background-position:center;position:relative;padding:50px;align-items:center}
.home-slide.active{display:grid;grid-template-columns:1fr 1fr;animation:fadeIn .45s ease}
.home-slide:before{content:"";position:absolute;inset:0;background:linear-gradient(90deg,rgba(255,250,244,.97),rgba(255,250,244,.65) 42%,rgba(20,17,15,.08))}
.home-slide-content{position:relative;z-index:2;animation:floatIn .75s ease both}
.home-kicker{display:inline-flex;background:rgba(16,16,16,.08);border:1px solid rgba(20,17,15,.12);border-radius:999px;padding:8px 13px;font-weight:900;font-size:12px;color:#4f4740;margin-bottom:12px}
.home-slide h1{font-family:Georgia,serif;font-size:clamp(54px,9vw,92px);letter-spacing:4px;margin:0;color:#14110f}
.home-slide h2{font-size:22px;margin:10px 0 0;color:#2c241d}.home-slide p{font-size:17px;line-height:2;color:#4f4740;max-width:520px}
.hero-btn{display:inline-flex;background:#101010;color:#fffaf4;border-radius:12px;padding:15px 24px;font-weight:900;margin-top:14px;transition:.25s}.hero-btn:hover,.add-btn:hover{transform:translateY(-3px)}
.slider-controls{position:absolute;z-index:5;left:22px;bottom:22px;display:flex;gap:8px;align-items:center}
.slider-dot{width:10px;height:10px;border-radius:999px;border:0;background:rgba(16,16,16,.28);transition:.25s}.slider-dot.active{width:30px;background:#101010}
.slider-arrow{position:absolute;z-index:5;top:50%;transform:translateY(-50%);width:42px;height:42px;border-radius:999px;border:1px solid rgba(255,255,255,.45);background:rgba(255,250,244,.78);backdrop-filter:blur(12px);font-size:22px;font-weight:900}.slider-arrow.prev{right:18px}.slider-arrow.next{left:18px}

.section-head{display:flex;align-items:end;justify-content:space-between;margin:38px 0 16px}.section-head h2{margin:0;font-size:25px}
.cat-grid{display:grid;grid-template-columns:repeat(6,1fr);gap:14px}.cat-card{background:#fffaf4;border:1px solid var(--line);border-radius:20px;overflow:hidden;box-shadow:var(--shadow);text-align:center;transition:.3s}.cat-card:hover,.product-card:hover{transform:translateY(-8px)}
.cat-img{height:145px;background-size:cover;background-position:center;transition:.5s}.cat-card:hover .cat-img,.product-card:hover .product-image{transform:scale(1.04)}.cat-card strong{display:block;padding:13px 0 4px}.cat-card span{display:block;color:#766f68;font-size:12px;padding-bottom:13px}
.products{display:grid;grid-template-columns:repeat(4,1fr);gap:16px}.product-card{position:relative;background:#fffaf4;border:1px solid var(--line);border-radius:20px;overflow:hidden;box-shadow:var(--shadow);transition:.3s}
.wish{position:absolute;z-index:2;top:10px;right:10px;width:36px;height:36px;border-radius:999px;border:0;background:rgba(255,255,255,.86);font-size:19px}.wish.active{background:#101010;color:#fff}
.product-image{display:block;height:260px;background-size:cover;background-position:center;position:relative;transition:.5s}.product-badge{position:absolute;top:10px;left:10px;background:#101010;color:#fffaf4;border-radius:999px;padding:6px 10px;font-size:11px;font-weight:900}
.product-info{padding:13px}.product-title{display:block;font-weight:900;margin-bottom:5px}.product-sub,.muted{color:var(--muted);line-height:1.9}.product-sub{font-size:12px;margin-bottom:8px}.product-price span{text-decoration:line-through;color:#9b9187;font-size:12px;margin-right:6px}.add-btn{width:100%;margin-top:11px;border:0;background:#101010;color:#fffaf4;border-radius:12px;padding:12px;font-weight:900;transition:.25s}
...
.trust-footer{display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin:24px auto}.trust-item,.trust-card{background:#fffaf4;border:1px solid var(--line);border-radius:18px;padding:18px;box-shadow:var(--shadow)}.trust-item{display:flex;gap:12px;align-items:center}
.trust-panel{display:grid;grid-template-columns:repeat(3,1fr);gap:12px;margin:18px 0}.payment-box,.panel{background:#fffaf4;border:1px solid var(--line);border-radius:24px;box-shadow:var(--shadow);padding:20px;margin:18px 0}
.gateway-row{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin:12px 0}.gateway{border:1px solid var(--line);border-radius:16px;background:#fff;padding:14px;text-align:center;font-weight:900}.gateway.active{background:#101010;color:#fff}
.shop-tools{display:grid;grid-template-columns:1fr auto;gap:12px;align-items:center}.shop-tools input,.input{width:100%;border:1px solid var(--line);border-radius:14px;background:#fff;padding:14px;outline:none}.filters{display:flex;gap:8px;overflow:auto}.filters a{white-space:nowrap;border:1px solid var(--line);border-radius:999px;padding:11px 14px;background:#fff}.filters a.active{background:#101010;color:#fff}
.product-page{display:grid;grid-template-columns:1.05fr .95fr;gap:18px}.big-img{min-height:560px;border-radius:24px;background-size:cover;background-position:center}.chips{display:flex;gap:8px;flex-wrap:wrap}.chip{border:1px solid var(--line);padding:10px 13px;border-radius:12px;background:#fff}
...
function moveHomeSlide(step){showHomeSlide(homeSlide+step)}
setInterval(()=>showHomeSlide(homeSlide+1),5500);
...
function updateCartCount(){const n=getCart().reduce((s,i)=>s+i.qty,0),el=document.getElementById("cartCount");if(el)el.textContent=n}
function addToCart(id,name,price,image){const c=getCart(),item=c.find(x=>x.id===id);if(item)item.qty++;else c.push({id,name,price,image,qty:1});setCart(c);alert("به سبد خرید اضافه شد")}
function changeQty(id,d){const c=getCart(),it=c.find(x=>x.id===id);if(!it)return;it.qty+=d;if(it.qty<=0)c.splice(c.indexOf(it),1);setCart(c);location.reload()}
function clearCart(){setCart([]);location.reload()}
function openDrawer(){drawer.classList.add("open");drawerBackdrop.classList.add("open")}function closeDrawer(){drawer.classList.remove("open");drawerBackdrop.classList.remove("open")}
function openAuth(){authModal.classList.add("open");authBackdrop.classList.add("open")}function closeAuth(){authModal.classList.remove("open");authBackdrop.classList.remove("open")}
function setAuthMode(mode){authMode=mode;registerTab.classList.toggle("active",mode==="register");loginTab.classList.toggle("active",mode==="login");authSubmit.textContent=mode==="register"?"ارسال کد تایید":"ورود";otpBox.style.display="none";authError.textContent="";authSuccess.textContent=""}
function validIranPhone(p){return /^09\\d{9}$/.test(String(p||"").trim())}function validPassword(p){return String(p||"").length>=8&&/[A-Za-z]/.test(p)&&/\\d/.test(p)}
...
function homeSlider(){
  return `<section class="home-slider" id="homeSlider">
    <article class="home-slide active" style="background-image:url('https://images.unsplash.com/photo-1509631179647-0177331693ae?auto=format&fit=crop&w=1800&q=88')">
      <div class="home-slide-content"><span class="home-kicker">NEW COLLECTION</span><h1>AESTRA</h1><h2>استایل تو، امضای توست</h2><p>کالکشن جدید آسترا برای ساختن یک ظاهر مدرن، لوکس و قابل اعتماد طراحی شده است.</p><a class="hero-btn" href="/shop?cat=women">مشاهده کالکشن زنانه</a></div>
    </article>
    <article class="home-slide" style="background-image:url('https://images.unsplash.com/photo-1515886657613-9f3515b0c78f?auto=format&fit=crop&w=1800&q=88')">
      <div class="home-slide-content"><span class="home-kicker">MEN STYLE</span><h1>MODERN</h1><h2>سادگی، قدرت، وقار</h2><p>استایل مردانه آسترا برای کسانی است که جزئیات را جدی می‌گیرند.</p><a class="hero-btn" href="/shop?cat=men">مشاهده کالکشن مردانه</a></div>
    </article>
    <article class="home-slide" style="background-image:url('https://images.unsplash.com/photo-1529139574466-a303027c1d8b?auto=format&fit=crop&w=1800&q=88')">
      <div class="home-slide-content"><span class="home-kicker">ACCESSORIES</span><h1>DETAILS</h1><h2>جزئیات، استایل را کامل می‌کنند</h2><p>از ساعت تا کیف و اکسسوری؛ انتخاب‌هایی برای تکمیل تصویر شخصی شما.</p><a class="hero-btn" href="/shop?cat=accessories">مشاهده اکسسوری‌ها</a></div>
    </article>
    <button class="slider-arrow prev" onclick="moveHomeSlide(-1)">›</button>
    <button class="slider-arrow next" onclick="moveHomeSlide(1)">‹</button>
...
app.get("/checkout",(req,res)=>res.send(shell(`<section class="payment-box reveal"><h1>پرداخت امن AESTRA</h1><p class="muted">قبل از پرداخت، سفارش شما با شماره موبایل تایید شده و از طریق درگاه معتبر بانکی انجام می‌شود.</p><div class="trust-panel"><div class="trust-card"><b>تایید پیامکی</b><p class="muted">ثبت‌نام با شماره موبایل ایران و کد تایید انجام می‌شود.</p></div><div class="trust-card"><b>درگاه معتبر</b><p class="muted">در نسخه نهایی به زرین‌پال یا درگاه بانکی وصل می‌شود.</p></div><div class="trust-card"><b>نماد اعتماد</b><p class="muted">جایگاه اینماد در فوتر سایت آماده شده است.</p></div></div><div class="gateway-row"><div class="gateway active">زرین‌پال / نمایشی</div><div class="gateway">درگاه بانکی / نمایشی</div></div><button class="add-btn" onclick="alert('پرداخت نمایشی موفق بود.');localStorage.removeItem('aestra_cart');location.href='/'">پرداخت نمایشی سفارش ${String(req.query.order||"")}</button></section>`,"cart")));

function page(t,txt,a="home"){return shell(`<section class="panel reveal"><h1>${t}</h1><p class="muted">${txt}</p></section>`,a)}
app.get("/about",(req,res)=>res.send(page("درباره AESTRA","AESTRA یک فروشگاه آنلاین مدرن برای پوشاک، ساعت، کیف، کفش و اکسسوری است. هدف ما ساختن تجربه‌ای ساده، شیک و قابل اعتماد برای انتخاب استایل شخصی شماست.")));
app.get("/contact",(req,res)=>res.send(shell(`<section class="panel reveal"><h1>تماس با ما</h1><p class="muted">آدرس: تهران، خیابان نمونه، پلاک ۱۲</p><p class="muted">شماره تماس: ۰۲۱-۰۰۰۰۰۰۰۰</p><input class="input" placeholder="نام شما"><br><br><input class="input" placeholder="شماره تماس"><br><br><input class="input" placeholder="پیام شما"></section>`)));
app.get("/account",(req,res)=>res.send(page("حساب کاربری","برای ثبت نام یا ورود، از دکمه ثبت نام / ورود در بالای سایت استفاده کنید.","account")));
app.get("/support",(req,res)=>res.send(page("پشتیبانی","پشتیبانی سفارش‌ها، راهنمای خرید و پیگیری ارسال از این بخش انجام می‌شود.")));
app.get("/ticket",(req,res)=>res.send(page("تیکت","در نسخه نهایی وردپرس، سیستم تیکت برای پیگیری درخواست‌های مشتری فعال می‌شود.")));
